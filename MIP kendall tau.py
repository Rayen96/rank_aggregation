...
def MIP(data):
    ...
    Q = m.addVars(n, n, vtype=GRB.INTEGER, name='Q')
    T = m.addVars(n, n, vtype=GRB.INTEGER, name='T')
    U = m.addVars(n, n, vtype=GRB.INTEGER, name='U')

    # Set objective
    m.setObjective(gp.quicksum(x[i, j]*better_than[j, i]+x[j, i]*better_than[i, j]+0.5*(better_than[i, j]+better_than[j, i])
                            * eq[i, j] for i in range(n) for j in range(i)), GRB.MINIMIZE)

    m.addConstr(P == gp.quicksum(x[i, j]*better_than[i, j] for i in range(n) for j in range(i)))
    m.addConstr(Q == gp.quicksum(x[i, j]*better_than[j, i] for i in range(n) for j in range(i)))

    m.addConstr(T == gp.quicksum(equal_to[i, j] for i in range(n) for j in range(i)))
